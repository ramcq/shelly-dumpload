// Thermal Dump Controller for Shelly Pro 2PM or Pro Dimmer 0/1-10V PM
// Controls pump and fan coil based on:
// 1. Frost thermostat input (unconditional)
// 2. Dump load relay states (voltage present, no consumption)
...
let config = {
  ...
  device: {
    type: "pro2pm",  // "pro2pm" (2 outputs) or "prodimmer" (1 output)
...
const OUTPUT_PUMP = 1;     // Output 2 (index 1) - not present on pro dimmer
...
function updateStatus(event) {
  ...
  let pumpPart = config.device.type === "pro2pm" ? ", Pump " + (state.pumpOn ? "ON" : "OFF") : "";
...
function updateOutputStates() {
  ...
  // Check output 1 (pump) - only if device has 2 outputs
  if (config.device.type === "pro2pm") {
    Shelly.call(
      "Switch.GetStatus",
      { id: OUTPUT_PUMP },
      function(result, error_code, error_message) {
...
function checkSystemState() {
  ...
  if (state.frostThermostatActive) {
    ...
    if (config.device.type === "pro2pm" && !state.pumpOn) {
  ...
  if (config.device.type === "pro2pm") {
    if (pumpConditionMet && !state.pumpOn) {
      turnOutputOn(OUTPUT_PUMP, "Pump", "Tank hot (" + state.topTankTemp.toFixed(1) + "°C), boiler off, dump active");
  ...
  if (fanConditionMet && !state.fanCoilOn) {
    turnOutputOn(OUTPUT_FAN_COIL, "Fan Coil", "Tank hot, low delta (" + tempDelta.toFixed(1) + "°C), dump active");
